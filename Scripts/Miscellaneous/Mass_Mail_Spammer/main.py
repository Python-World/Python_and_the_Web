...
def check_err(email, target, spam, choice):
    ...
    try:
        ...
        print("Number of spam messages/choice should be a positive integer\nExiting...\n")
        sys.exit()
...
def rand_msg(spam):
    ...
    for i in range(int(spam)):
        ...
        msg.append(''.join(random.choice(string.ascii_letters + string.digits) for _ in range(N)))
...
"""
    ...
    for i in range(int(spam)):
        subprocess.check_call(['./expect_mail.sh', msg[i], sub[i], encoded_creds, target, email])
        sleep(1)
