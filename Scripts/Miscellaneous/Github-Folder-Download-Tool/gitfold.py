...
def get_folder_links(folder_url):
    ...
    _all_folder_folders_href = [link for link in _all_folder_links if "blob" not in link] # extracts all file links
    print("Files -- ",_all_folder_links_href,"\n")
    print("Folders -- ",_all_folder_folders_href, "\n")
    # exit()
    return _all_folder_folders_href, _all_folder_links_href # returns both Folders links and Files links as a list
...
# creates a mirror to disk, thereby preserving the folders hierechical structure as it is on github
    if len(folder_links_received) > 0:
        download_files(file_hrefs_received) # if files present in folder, download them first
        for folder in folder_links_received:
            print("folder - ", folder)
            folder_name = str(folder).split('/')[-1]
            mkdir(folder_name) # creates a local copy|mirror of the currently walked|selected github folder. ... helps preserve folder structure
